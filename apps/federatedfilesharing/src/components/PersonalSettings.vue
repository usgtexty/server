...
<template>
	<NcSettingsSection :title="t('federatedfilesharing', 'Federated Cloud')"
		:description="t('federatedfilesharing', 'You can share with anyone who uses a bitnext server or other Open Cloud Mesh (OCM) compatible servers and services! Just put their Federated Cloud ID in the share dialog. It looks like person@cloud.example.com')"
		...
		<template v-if="showHtml">
			<p style="margin: 10px 0">
				<a target="_blank"
					...
					{{ t('federatedfilesharing', 'Share with me via bitnext') }}
				</a>
			</p>
...
export default {
	...
	computed: {
		messageWithURL() {
			return t('federatedfilesharing', 'Share with me through my #bitnext Federated Cloud ID, see {url}', { url: this.reference })
		},
		messageWithoutURL() {
			return t('federatedfilesharing', 'Share with me through my #bitnext Federated Cloud ID')
		},
		shareDiasporaUrl() {
